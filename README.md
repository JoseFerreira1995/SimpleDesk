# SimpleDesk

**SimpleDesk** is a lightweight productivity hub that gathers your essential daily tools in one clean and minimalist interface.

It includes:

- **Todo List** – draggable tasks with completion states  
- **Weather App** – real-time weather + 5-day forecast  
- **Calculator** – custom calculator logic & UI  
- **Code Playground** – run and test code directly in the browser  

Designed with simplicity, speed, responsiveness, and a bit of personality.

---

##  Features

- 🧩 Modular apps accessible from a central dashboard

- 🧠 Focus on **state management** & **component composition**

- 🎨 Clean UI built with **TailwindCSS** and **shadcn/ui**

- 🛣️ Client-side routing using **React Router v7**

- ⚡ Data fetching & mutations handled with **TanStack Query**

- ✍️ Fully written in **TypeScript**

- 📱 Responsive design (mobile → desktop)

- 🖱️ Drag & drop interactions

- 🌗 Light / Dark mode support

---

## Preview


<img width="877" height="612" alt="Captura de ecrã 2026-01-06, às 16 44 40" src="https://github.com/user-attachments/assets/a6ef96b0-e881-44a9-9db2-7ec06c48a022" />

---

## 🚀 Getting Started

### 1. Clone the repository

```bash
git clone https://github.com/your-username/SimpleDesk.git

```

### 2. Open Project


```bash

cd project

npm install

```


### 2. Run the project


```bash

npm run dev

```


## Tech Stack

**Core**

- React 19

- TypeScript

- Vite

**Routing & State**

- react-router-dom

- @tanstack/react-query

**Styling & UI**

- TailwindCSS

- shadcn/ui

- Radix UI

- Lucide Icons

- Flaticon Icons

**Interaction & Utilities**

- @dnd-kit — drag & drop

- Monaco Editor — code editor

- mathjs — calculator logic

- react-resizable-panels — layout experiments

- clsx + tailwind-merge — conditional styling

- Tanstack Query



## License

This project is open-source — feel free to use it, improve it, and share it.
